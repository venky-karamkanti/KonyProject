...
function isUnixOS() {
     return ['darwin', 'linux'].includes(os.platform().toLowerCase());
}
...
function __preBuildValidation__(params) {
    ...
    if(projectName.length > 17 || projectName.length < 3) {
        throw new BuildException(`ProjectName should be between 3 and 17 characters`, 50);
    }

    let appid = readProp('appid');
    if(appid.length > 32 || appid.length < 4) {
        throw new BuildException(`appid should be between 4 and 32 characters`, 50);
    }
...
function extractPluginsAndBuildProject(args) {
    ...
    var ci_platform = (isUnixOS()) ? 'mac64' : 'win64',
